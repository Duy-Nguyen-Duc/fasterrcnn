...
def return_fasterrcnn_pvig_tiny(
    num_classes, pretrained=True, coco_model=False
):
    model = fasterrcnn_pvig_tiny.create_model(
        num_classes, pretrained, coco_model=coco_model
    )
    return model
...
def return_fasterrcnn_vgg16_fpn(
    num_classes, pretrained=True, coco_model=False
):
    model = fasterrcnn_vgg16_fpn.create_model(
        num_classes, pretrained, coco_model=coco_model
    )
    return model
...
create_model = {
    ...
    'fasterrcnn_vgg16_fpn': return_fasterrcnn_vgg16_fpn,
}